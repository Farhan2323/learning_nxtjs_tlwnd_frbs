...
export default function Home() {
  const [items, setItems] = useState([
    ...
    // {name:'bagel' , price: 6.50},
    // {name:'pastry' , price: 4.90},
  ...
  // delete from DB (usually not supposed to be doing as good practice of DB management)
        ...
        <div className = "bg-slate-800 p-4 rounded-xl" >
          <form className = "grid grid-cols-6 items-center text-black">
            ...
            <button onClick={addItem}
                    className="text-white bg-slate-950 hover:bg-slate-800 p-3 text-xl" 
                    type="submit"> + </button>
          </form>
          ...
          <ul>
            {items.map((item, id) =>(
              <li key={id} 
                ...
                <button onClick={() => deleteItem(item.id)} className="ml-8 p-4 border-l-2 border-slate-500 hover:bg-slate-800 w-16"> X</button>
              </li>
              ))}
          </ul>
          {items.length < 1 ? ('') : (
